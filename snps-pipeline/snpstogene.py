...
def snpstogene(snpspath: Path, locmappath: Path, isolates: list) -> pd.DataFrame:
	...
	snpgenes.columns = ['Chr', 'SNPPos', 'REF', 'ALT'] + isolates + [f"{isolate}_condition" for isolate in isolates] + ['Gene', 'Start', 'End']
	snpgenes = snpgenes[['Chr', 'SNPPos', 'Gene', 'Start', 'End', 'REF', 'ALT'] + [f"{isolate}_condition" for isolate in isolates] + isolates]
